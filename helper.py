...
def getWordScore(word, n):
    ...
    for i in range(0,len(word)):
        alphabet=word[i]
        value += int(SCRABBLE_LETTER_VALUES[alphabet])
...
def updateHand(hand, word):
   ...
   # print(hand)
    for i in word:
        alphabetIndex=newHand.get(i,0)
        if(alphabetIndex != 0):
            alphabetIndex -= 1
        newHand[i]=alphabetIndex
    return newHand
...
def playHand(hand, wordList, n):
    ...
    * The hand finishes when there are no more unused letters or the user
    ...
    if(word=='.'):print('Goodbye! Total score: %d points. ' %score)
    else:print('Run out of letters. Total score: %d points. ' %score)
